...
const DEFAULT_MODEL = (Deno.env.get("MOM_MODEL") ?? "Qwen/Qwen2.5-7B-Instruct").trim();
...
Deno.serve(async (req: Request) => {
  ...
  const system = [MOM_RUNTIME_GUARD, systemPrompt, awareness].filter(Boolean).join("\n\n");
  ...
  if (!provider.ok || !provider.body) {
    ...
    return json(502, { error: "provider_error", provider_status: provider.status, detail: detail.slice(0, 2000) });
  }
